...
/**
 * Build a provisional frozen scale from current item parameters and neutral
 * defaults when a tryout does not yet qualify for a fully calibrated official
 * snapshot.
 */
export async function buildProvisionalScaleItems(
  db: IrtDbWriter,
  {
    now,
    tryoutId,
  ...
  for (const { partSet, set } of tryoutSets) {
    ...
    const provisionalRunId = await createProvisionalCalibrationRun(db, {
      now,
    ...
    for (const question of questions) {
      ...
      items.push({
        ...
        calibrationRunId: params?.calibrationRunId ?? provisionalRunId,
      });
    }
  }
