...
/** Builds a complete curriculum topic sync payload with focused overrides. */
function buildTopic(
  overrides: Partial<SyncedCurriculumTopic> = {}
): SyncedCurriculumTopic {
  return { ...BASE_TOPIC, ...overrides };
}

/** Builds a complete curriculum lesson sync payload with focused overrides. */
function buildSection(
  overrides: Partial<SyncedCurriculumLesson> = {}
): SyncedCurriculumLesson {
  return { ...BASE_SECTION, ...overrides };
}

/** Returns the graph asset ID for a curriculum route fixture. */
...
describe("contentSync/mutations/curriculum", () => {
  it("syncs curriculum topics through create, unchanged, and update paths", async () => {
  ...
  it("syncs curriculum lessons, skips missing topics, and updates rendered metadata", async () => {
    const t = convexTest(schema, convexModules);
    ...
    const created = await t.mutation(
      ...
      {
        sections: [
          buildSection({ topicSlug: "material/lesson/mathematics/missing" }),
          buildSection(),
        ],
      }
    );
    ...
    expect(created).toEqual({
      ...
      created: 1,
      skipped: 1,
      skippedTopicSlugs: ["material/lesson/mathematics/missing"],
      unchanged: 0,
      updated: 0,
    });
    ...
    expect(snapshot.search?.text).not.toContain("material/lesson");
  ...
  it("returns empty summaries for empty curriculum sync and stale delete batches", async () => {
    const t = convexTest(schema, convexModules);
  ...
  it("deletes stale curriculum lessons and skips IDs that already disappeared", async () => {
    ...
    const ids = await t.mutation(async (ctx) => {
      ...
      if (!(section && topic)) {
        throw new Error(
          "Expected synced curriculum lesson before stale delete."
        );
      }
      ...
      const missingId = await ctx.db.insert("curriculumLessons", {
        ...
        order: 9,
        section: "missing",
        slug: "material/lesson/mathematics/metadata-topic/missing",
        syncedAt: 1,
        title: "Missing",
        topic: "metadata-topic",
        topicId: topic._id,
    ...
    const snapshot = await t.query(async (ctx) => {
      ...
      const authorLinks = await ctx.db
        .query("contentAuthors")
        .withIndex("by_contentId_and_contentType_and_authorId", (q) =>
          q.eq("contentId", ids.sectionId).eq("contentType", "material")
        )
        .collect();
      const search = await ctx.db
        .query("contentSearch")
  ...
  it("deletes stale curriculum topics with their sections and rejects unsafe section counts", async () => {
    const t = convexTest(schema, convexModules);
    ...
    const ids = await t.mutation(async (ctx) => {
      ...
      if (!(topic && unsafeTopic)) {
        throw new Error(
          "Expected synced curriculum topics before stale delete."
        );
      }
      ...
      await ctx.db.patch("curriculumTopics", unsafeTopic._id, {
        sectionCount: 0,
      });
      ...
      await ctx.db.delete("curriculumTopics", missingId);
