...
import { migrateMaterialCatalog } from "@repo/backend/convex/contentRelease/material/migrate";
...
describe("contentRelease/material/migrate", () => {
  ...
  it("rejects missing or excessive discovery bucket state", async () => {
    const missingTarget = convexTest(schema, convexModules);
    await missingTarget.mutation((ctx) =>
    ...
    const excessiveTarget = convexTest(schema, convexModules);
    await excessiveTarget.mutation(async (ctx) => {
      for (let index = 0; index <= 100; index += 1) {
