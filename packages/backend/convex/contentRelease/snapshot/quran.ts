...
export const stageQuranRow = Effect.fn("contentRelease.stageQuranRow")(
  function* (
    ...
    const row = {
      firstVerse:
        payload.kind === "quran-chunk" ? payload.firstVerse : undefined,
      identity,
      index,
      kind: payload.kind,
      locale: payload.kind === "quran-search" ? payload.locale : undefined,
      rowHash: source.record.rowHash,
      rowJson,
      snapshotId,
      surahNumber:
        payload.kind === "quran-surah" ? payload.number : payload.surahNumber,
    };
