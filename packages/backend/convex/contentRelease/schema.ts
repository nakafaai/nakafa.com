...
const tables = {
  ...
  contentReleases: defineTable({
    ...
    baseManifestHash: v.optional(v.union(v.string(), v.null())),
    baseReleaseId: v.optional(v.union(v.string(), v.null())),
    ...
    originKind: v.optional(v.union(v.literal("git"), v.literal("rollback"))),
    ...
    rendererManifestHash: v.optional(v.string()),
    resultFamilies: v.array(contentFamilyValidator),
    role: releaseRoleValidator,
    sequence: v.number(),
    snapshotTransitions: v.optional(releaseSnapshotTransitionsValidator),
