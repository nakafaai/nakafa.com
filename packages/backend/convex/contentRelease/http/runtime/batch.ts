...
/** Authenticates and forwards one bounded public runtime batch. */
const publicRuntimeBatchRoute = Effect.fn(
  "contentRelease.publicRuntimeBatchRoute"
...
export function registerPublicContentRuntimeBatchRoute(
  ...
  app.post(PUBLIC_CONTENT_RUNTIME_BATCH_PATH, async (context) => {
    const result = await runConvexProgram(
      publicRuntimeBatchRoute(context.env, context.req.raw)
    );
