...
/** Builds the search schema input from typed public query parameters. */
export const readSearchInput = Effect.fn("agent.http.readSearchInput")(
  function* (url: URL) {
    ...
    const [limit, locale, offset, section] = yield* Effect.all([
      ...
      readOptionalSingleton(url, "locale"),
      readOptionalInteger(url, "offset"),
      readOptionalSingleton(url, "section"),
    ]);
    return compactObject({
      limit,
      locale,
      offset,
      queries: queries.length === 0 ? undefined : queries,
      section,
    });
  }
);
...
export const readContentInput = Effect.fn("agent.http.readContentInput")(
  function* (url: URL) {
    ...
    const ref = yield* readOptionalSingleton(url, "ref");
    if (ref) {
      return ref;
    }
...
/** Builds the taxonomy schema input from the optional locale. */
export const readTaxonomyInput = Effect.fn("agent.http.readTaxonomyInput")(
  function* (url: URL) {
    yield* assertAllowedParameters(url, ["locale"]);
    const locale = yield* readOptionalSingleton(url, "locale");
    return compactObject({ locale });
  }
);

/** Builds the Quran schema input from its path and query parameters. */
...
/** Detects a body whose declared media type cannot be consumed by this API. */
export function hasUnsupportedRequestMediaType(request: Request) {
  const contentLength = Number(request.headers.get("content-length") ?? "0");
  const hasDeclaredBody = Number.isFinite(contentLength) && contentLength > 0;
  if (request.body === null && !hasDeclaredBody) {
    return false;
  }
...
function readOptionalSingleton(url: URL, name: string) {
...
const readOptionalInteger = Effect.fn("agent.http.readOptionalInteger")(
  function* (url: URL, name: string) {
    const value = yield* readOptionalSingleton(url, name);
...
/** Parses one canonical base-10 integer without coercing partial values. */
...
/** Removes undefined fields before Effect Schema applies defaults. */
function compactObject(values: {
  readonly [key: string]: boolean | number | string | string[] | undefined;
}) {
