...
/** Deletes one bounded page from an unreferenced migrated history scale. */
export const cleanupTryoutHistoryScale = Effect.fn(
  "tryouts.history.cleanupScale"
  ...
  if (!scale) {
    return yield* new TryoutRuntimeError({
      ...
      message: "Retained try-out history lost its IRT scale.",
    });
  }
