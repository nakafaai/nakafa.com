...
describe("tryouts/response/migration/program", () => {
  ...
  it("rejects a placement whose predecessor and replacement differ", async () => {
    ...
    await seedResponseMigrationFixture(t, 1);
    ...
    ).rejects.toMatchObject(migrationFailure);
  });
  ...
  it("refuses contraction when the canonical score is stale", async () => {
    const t = createConvexTestWithBetterAuth();
    await seedResponseMigrationFixture(t, 1);
    await hydrate(t);
    const responseId = await t.mutation(async (ctx) => {
      const responses = await ctx.db.query("tryoutResponses").collect();
      const response = responses.find(
  ...
  it("rejects a response whose placement no longer exists", async () => {
    const t = createConvexTestWithBetterAuth();
    await seedInvalidResponseMigration(t, "placement", ({ selected }) => ({
  ...
  it("rejects a canonical response linked to another attempt", async () => {
    const t = createConvexTestWithBetterAuth();
    await seedResponseMigrationFixture(t, 1);
    await hydrate(t);
    await t.mutation(async (ctx) => {
  ...
  it("rejects a canonical response linked to another section", async () => {
    const t = createConvexTestWithBetterAuth();
    await seedResponseMigrationFixture(t, 1);
    await hydrate(t);
    await t.mutation(async (ctx) => {
  ...
  it("rejects a response with no supported learner selection", async () => {
    const t = createConvexTestWithBetterAuth();
    await seedInvalidResponseMigration(t, "empty", ({ selected }) => ({
      isCorrect: selected.isCorrect,
    }));
...
async function hydrate(t: ConvexTest) {
