...
describe("tryouts/queries/attempt", () => {
  it("binds exact navigation to an unmigrated filesystem attempt", async () => {
    ...
    const identity = await t.mutation(async (ctx) => {
      const user = await seedAuthenticatedUser(ctx, {
        ...
        suffix: "route-bound-legacy-attempt",
      });
    ...
    const content = await authed.query(
    ...
    expect(latest?.attemptId).toBe(started.attemptId);
    expect(exact?.attemptId).toBe(started.attemptId);
    expect(exact?.sectionRoutes).toEqual(latest?.sectionRoutes);
    expect(page?.attemptId).toBe(started.attemptId);
    expect(runtime?.attemptId).toBe(started.attemptId);
    ...
    await t.mutation(async (ctx) => {
      ...
      if (!(snapshot?.questionSetId && snapshot.tryoutSectionId)) {
        throw new Error("Expected one frozen filesystem section source.");
      }
      const questionSet = await ctx.db.get(snapshot.questionSetId);
      if (!questionSet) {
        throw new Error("Expected the frozen question set fixture.");
      }
      const { _creationTime, _id, ...fields } = questionSet;
      const replacementQuestionSetId = await ctx.db.insert("questionSets", {
        ...fields,
        contentHash: "replacement-question-set-hash",
      });
      await ctx.db.patch(snapshot.tryoutSectionId, {
        questionSetId: replacementQuestionSetId,
      });
    });
    await expect(
