...
/** Builds one subscription payload with a stable timestamp window. */
...
async function insertSubscription(ctx: MutationCtx, input: SubscriptionInput) {
  return await ctx.db.insert("subscriptions", buildSubscription(input));
}
