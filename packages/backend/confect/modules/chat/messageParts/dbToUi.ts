...
import { restoreProviderMetadata } from "@repo/backend/confect/modules/chat/messageParts/providerMetadata";
import {
  failMessagePart,
  requirePartField,
  requireToolState,
...
/** Converts one persisted Convex part into the AI SDK UI message shape. */
function mapDBPartToUIMessagePart({
  part,
}: {
  readonly part: Doc<"parts">;
}): MyUIMessagePart {
  switch (part.type) {
    case "text":
      return {
    ...
    case "reasoning":
      return {
        ...
        providerMetadata: restoreProviderMetadata(part.providerMetadata),
      };
    case "file":
      return {
        type: part.type,
      ...
      switch (toolState) {
        case "input-streaming":
          return {
            ...
            callProviderMetadata: restoreProviderMetadata(
              part.toolCallProviderMetadata
            ),
        ...
        case "output-available":
          return {
            ...
            resultProviderMetadata: restoreProviderMetadata(
              part.toolResultProviderMetadata
            ),
          };
        case "output-error":
          return {
        ...
        default:
          return failMessagePart(
            `Unsupported persisted tool state: ${toolState}`
          );
      }
    }
    ...
    default:
      return failMessagePart(`Unsupported persisted part type: ${part.type}`);
  }
}

export { mapDBPartToUIMessagePart };
