...
export const getClasses = Effect.fnUntraced(function* (args: {
  isArchived?: boolean;
...
export const getClassRoute = Effect.fnUntraced(function* (args: {
  classId: string;
}) {
  const user = yield* requireAppUser();
...
export const getPeople = Effect.fnUntraced(function* (args: {
  ...
  if (normalizedQuery) {
    const expectedMemberCount = classData.studentCount + classData.teacherCount;
...
export const getInviteCodes = Effect.fnUntraced(function* (args: {
  classId: Id<"schoolClasses">;
}) {
  const user = yield* requireAppUser();
  const classData = yield* loadClass(args.classId);
  ...
  const inviteCodes = yield* reader
    ...
    .index("by_classId_and_role", (index) => index.eq("classId", args.classId))
