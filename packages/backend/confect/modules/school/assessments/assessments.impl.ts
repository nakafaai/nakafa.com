...
import { createAssignment } from "@repo/backend/confect/modules/school/assessmentsAuthoring/assignments.service";
import {
  createQuestionBank,
  createQuestionBankEntry,
} from "@repo/backend/confect/modules/school/assessmentsAuthoring/bank.service";
import { deleteAssessment } from "@repo/backend/confect/modules/school/assessmentsAuthoring/delete.service";
import {
  createAssessment,
  updateAssessment,
} from "@repo/backend/confect/modules/school/assessmentsAuthoring/drafts.service";
import { reorderAssessment } from "@repo/backend/confect/modules/school/assessmentsAuthoring/ordering.service";
import { publishAssessment } from "@repo/backend/confect/modules/school/assessmentsAuthoring/publishing.service";
import {
  createQuestion,
  createSection,
} from "@repo/backend/confect/modules/school/assessmentsAuthoring/questions.service";
import { createAssessmentVersion } from "@repo/backend/confect/modules/school/assessmentsAuthoring/versions.service";
import {
...
const assessments_mutations_public_bank_createQuestionBankEntryImpl =
  FunctionImpl.make(
    ...
    "createQuestionBankEntry",
    (args) =>
      createQuestionBankEntry(args).pipe(
        Effect.catchTags({
          AssessmentError: (error) => Effect.die(error),
          ClassActionError: (error) => Effect.die(error),
          UnauthorizedUser: (error) => Effect.die(error),
...
const assessments_mutations_public_version_createAssessmentVersionImpl =
  FunctionImpl.make(
    ...
    (args) =>
      createAssessmentVersion(args).pipe(
        Effect.catchTags({
          AssessmentError: (error) => Effect.die(error),
          ClassActionError: (error) => Effect.die(error),
          UnauthorizedUser: (error) => Effect.die(error),
...
const assessments_queries_public_authoring_getAuthoredAssessmentImpl =
  FunctionImpl.make(
    ...
    "getAuthoredAssessment",
    (args) =>
      getAuthoredAssessment(args).pipe(
        Effect.catchTags({
          ClassActionError: (error) => Effect.die(error),
          UnauthorizedUser: (error) => Effect.die(error),
        }),
