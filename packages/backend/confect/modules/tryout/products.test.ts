...
describe("tryout products", () => {
  it.effect("exposes the supported product and route guard", () =>
    Effect.sync(() => {
  ...
  it.effect("detects complete SNBT tryouts and orders parts by policy", () =>
    Effect.sync(() => {
  ...
  it.effect("ignores incomplete or non-SNBT candidate sets", () =>
    Effect.sync(() => {
      const policy = tryoutProductPolicies.snbt;
      const detected = policy.detectTryouts({
        locale: "en",
  ...
  it.effect(
    "formats leaderboard namespaces, scoring, time limits, and sort order",
    () =>
      Effect.sync(() => {
        const policy = tryoutProductPolicies.snbt;
        const detected = policy.detectTryouts({
  ...
  it.effect("sorts same-cycle tryouts by label and slug", () =>
    Effect.sync(() => {
      const policy = tryoutProductPolicies.snbt;
      const detected = policy.detectTryouts({
        locale: "id",
