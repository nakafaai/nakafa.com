...
export const loadCustomerEnvProvider = Effect.fn(
  ...
  return ConfigProvider.orElse(shell, () => ConfigProvider.fromMap(backend));
});
...
const getLocalAccessToken = Effect.fn("customers.getLocalAccessToken")(
  function* () {
    ...
    const config = yield* Schema.decodeUnknown(ConvexAuthConfigSchema)(
      json
    ).pipe(
      Effect.mapError(
        () =>
  ...
  Effect.gen(function* () {
      ...
      Effect.mapError(
        (error) =>
          new CustomerConvexResponseError({
            message: `Invalid Convex response: ${ParseResult.TreeFormatter.formatErrorSync(error)}`,
          })
      )
    );
    ...
    return yield* Schema.decodeUnknown(valueSchema)(response.value).pipe(
      Effect.mapError(
        (error) =>
          new CustomerConvexResponseError({
            message: `Invalid Convex value: ${ParseResult.TreeFormatter.formatErrorSync(error)}`,
          })
      )
    );
  });
...
export const callCustomerIntegrityQuery = Effect.fn(
  ...
  schema: Schema.Schema<FunctionReturnType<TQuery>, Encoded, never>
) {
