...
export const EXERCISE_MATERIAL_CONST_REGEX =
  /const\s+\w+Materials[^=]*=\s*(\[[\s\S]*?\])\s*(?:as\s+const|satisfies\s+ExercisesMaterialList)/;
...
export const SUBJECT_MATERIAL_CONST_REGEX =
  /const\s+\w+Materials[^=]*=\s*(\[[\s\S]*\])(?:\s*as\s+const|\s*satisfies\s+MaterialList)?;\s*export\s+default/;
