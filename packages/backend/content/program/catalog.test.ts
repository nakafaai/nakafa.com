...
describe("contentRelease/program/catalog", () => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const data = yield* makeProgramSnapshotData([
          {
            ...program,
            navigation: { ...program.navigation, model: "exam-domain-set" },
          },
        ]);
  ...
  it("returns an empty unmanaged catalog before program publication", async () => {
    ...
    await expect(
      t.query((ctx) =>
        runConvexProgram(
          readProgramCatalog("en").pipe(Effect.provide(convexProgramLayer(ctx)))
        )
      )
