    ...
    Effect.fn("contentRelease.program.route.test.returnsExactRoute")(
      function* () {
        ...
        const result = yield* Effect.promise(() =>
          t.query((ctx) =>
            runConvexProgram(
              readProgramRoute("en", "curriculum/technical-program-1").pipe(
    ...
    Effect.fn("contentRelease.program.route.test.distinguishesMissingRoute")(
      function* () {
        ...
        const result = yield* Effect.promise(() =>
          t.query((ctx) =>
            runConvexProgram(
              readProgramRoute("id", "kurikulum/missing").pipe(
                Effect.provide(convexProgramLayer(ctx))
              )
            )
          )
    ...
    Effect.fn("contentRelease.program.route.test.omitsRemovedMaterials")(
      function* () {
        ...
        const result = yield* Effect.promise(() =>
          target.query((ctx) =>
            runConvexProgram(
              readProgramRoute("en", PROGRAM_ROOT).pipe(
                Effect.provide(convexProgramLayer(ctx))
              )
            )
          )
      ...
      function* () {
        ...
        expect(groups).toMatchObject([
          ...
          },
          {
            ...
            record: {
              kind: "curriculum",
              row: { publicPath: tiedGroup.publicPath },
            },
          },
        ]);
      }
