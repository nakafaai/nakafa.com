...
// Constants for unique ID generation
const ALPHANUMERIC_BASE = 36;
const RANDOM_START_INDEX = 2; // Skip "0." prefix
const RANDOM_END_INDEX = 9;
  ...
  useEffect(() => {
    const renderChart = async () => {
      try {
        ...
        const uniqueId = `mermaid-${Math.abs(chartHash)}-${Date.now()}-${Math.random().toString(ALPHANUMERIC_BASE).substring(RANDOM_START_INDEX, RANDOM_END_INDEX)}`;
