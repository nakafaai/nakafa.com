...
/**
 * Derives the allowed swipe directions from a fixed viewport position.
 * Centered stacks should dismiss vertically; corner stacks also allow edge swipes.
 */
...
/**
 * Chooses the re-notification animation for upserted toasts.
 * Base UI increments updateKey when a stable id is reused.
 */
...
/**
 * Renders global stacked COSS toasts for the configured viewport.
 * It owns the viewport only; callers enqueue messages through toastManager.
 */
function Toasts({
  position,
  portalProps,
...
/**
 * Renders contextual COSS toasts anchored to a caller-provided element.
 * Toasts without an anchor are ignored so global messages stay in Toasts.
 */
...
/**
 * Provides the global COSS toast manager and viewport.
 * Nakafa defaults to bottom-center because existing app flows expect that spot.
 */
...
/**
 * Provides the anchored COSS toast manager for element-tethered feedback.
 * This is separate from ToastProvider so contextual toasts never pollute globals.
 */
