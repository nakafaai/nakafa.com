...
export const LiveMicrophoneWaveform = ({
  ...
  useEffect(() => {
    ...
    const drawBar = (
      ...
      const barHeight = Math.max(baseBarHeight, value * height * 0.7);
      const y = centerY - barHeight / 2;
    ...
    const drawBars = (
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        drawBar(ctx, value, x, centerY, rect.height, color);
      }
    };

    const drawFadeEdges = (ctx: CanvasRenderingContext2D, rect: DOMRect) => {
      if (!(fadeEdges && fadeWidth > 0)) {
        return;
      }
    ...
    const animate = (currentTime: number) => {
      ...
      drawBars(ctx, rect, color);
      drawFadeEdges(ctx, rect);
  ...
  useEffect(() => {
    ...
    const calculateStartIndex = (
      ...
      if (!recording && isRecordingComplete) {
        const totalBars = dataLength;
        if (totalBars > barsVisible) {
          return Math.floor((totalBars - barsVisible) * viewPosition);
        }
        return 0;
      }
      if (recording) {
    ...
    const drawRecordingBars = (
      ...
      const centerY = rect.height / 2;
      const startIndex = calculateStartIndex(dataToRender.length, barsVisible);
      ...
      for (
        ...
        const x = i * step;
        drawRecordingBar(ctx, value, x, centerY, rect.height, color);
      }
    };
    ...
    const animate = (currentTime: number) => {
      ...
      const color = getRecordingBarColor();
      drawRecordingBars(ctx, rect, color);
      drawPositionIndicator(ctx, rect, color);
