 ...
 */
function ClimateSymptomsScene({
  colors,
  item,
  selectedIndex,
}: BiologySceneProps) {
  if (selectedIndex === 1) {
    return <OceanHeat colors={colors} label={item.label} />;
  }

  if (selectedIndex === 2) {
    return <IceAndSeaLevel colors={colors} label={item.label} />;
  }

  return <TemperatureIndicator colors={colors} label={item.label} />;
}
  ...
  return (
    <group>
      ...
      {[-0.8, -0.25, 0.35, 0.9].map((x, index) => (
        <mesh key={x} position={[x, -0.2 + index * 0.12, 0.34]}>
