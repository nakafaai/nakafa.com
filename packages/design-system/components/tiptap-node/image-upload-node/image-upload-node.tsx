...
import { CloseIcon } from "@repo/design-system/components/tiptap-icons/close-icon";
import { Button } from "@repo/design-system/components/tiptap-ui-primitive/button";
import type { NodeViewProps } from "@tiptap/react";
import { NodeViewWrapper } from "@tiptap/react";
import { useRef, useState } from "react";
import "@repo/design-system/components/tiptap-node/image-upload-node/image-upload-node.scss";
import {
  focusNextNode,
  isValidPosition,
    ...
    if (urls.length > 0) {
      ...
      if (isValidPosition(pos)) {
        const imageNodes = urls.map((url, index) => {
          const filename =
            files[index]?.name.replace(FILE_EXTENSION_REGEX, "") || "unknown";
          return {
