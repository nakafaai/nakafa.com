...
export const mathProbability =
  "Use for supported named probability distributions such as normal, binomial, or poisson. Prefer this over arithmetic when a distribution and its parameters are given. For fair dice, cards, or any finite equally likely outcome list, use statistics mean or arithmetic over the listed outcomes instead. Always include required parameters: bernoulli p, binomial n and p, normal mean and standard_deviation, poisson lambda, uniform lower and upper. Use distribution when the user asks to inspect, check, or validate the distribution. Use expected_value or variance_probability when the user asks for expectation or variance.";
