...
const booleanStringSchema = Schema.UndefinedOr(
  Schema.Literals(["true", "false"])
);
const secretSchema = Schema.toStandardSchemaV1(
  Schema.Trimmed.check(Schema.isNonEmpty())
);
const vercelEnvSchema = Schema.toStandardSchemaV1(
  Schema.UndefinedOr(Schema.Literals(["development", "preview", "production"]))
);
/**
...
export const devtoolsKeys = () =>
  createEnv({
    server: {
      AI_SDK_DEVTOOLS: Schema.toStandardSchemaV1(booleanStringSchema),
      NODE_ENV: Schema.toStandardSchemaV1(nodeEnvSchema),
