...
describe("bounded response body", () => {
  it.each([
    ...
    ["0008", 8],
    ["8", 8],
  ] as const)("parses the optional Content-Length %s", async (value, expected) => {
    await expect(Effect.runPromise(parseContentLength(value, 8))).resolves.toBe(
      expected
    );
  });
  ...
  it.each([
    ...
    "1, 1",
    "9".repeat(400),
  ])("rejects the malformed Content-Length %s", async (value) => {
    await expect(
  ...
  it.each([
    false,
    true,
  ])("cancels an oversized body even when cancellation rejects: %s", async (rejectCancellation) => {
    const cancel = vi.fn(() =>
      rejectCancellation
