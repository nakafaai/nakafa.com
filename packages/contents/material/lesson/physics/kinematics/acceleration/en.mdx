...
<AccelerationLab
  ...
  description={
    <>
      A chase camera follows the rocket while{" "}
      <InlineMath math="1\text{ s}" /> time gates spread out, stay even, or
      bunch together.
    </>
  }
  labels={{
...
<AccelerationGraphCard
  ...
  description={
    <>
      Choose one segment on the same graph, then read the line slope as
...
On the graph, the dashed line shows the full connected graph. The thick line is the segment being read. If the line rises, velocity increases. If it stays flat, velocity is constant. If it slopes downward, velocity decreases. Acceleration is the ratio of <InlineMath math="\Delta v" /> to <InlineMath math="\Delta t" />.
...
Suppose a car changes velocity from <InlineMath math="2\text{ m/s}" /> to <InlineMath math="18\text{ m/s}" /> in <InlineMath math="4\text{ s}" />. Its acceleration is:
...
That result means the velocity increases by <InlineMath math="4\text{ m/s}" /> each second. The sequence becomes <InlineMath math="2\text{ m/s}" />, <InlineMath math="6\text{ m/s}" />, <InlineMath math="10\text{ m/s}" />, and so on if the acceleration remains constant.

Now compare it with a car whose velocity changes from <InlineMath math="18\text{ m/s}" /> to <InlineMath math="2\text{ m/s}" /> in <InlineMath math="4\text{ s}" />.
...
The value <InlineMath math="-4\text{ m/s}^2" /> means the velocity decreases by <InlineMath math="4\text{ m/s}" /> each second if the car is still moving in the positive direction.
