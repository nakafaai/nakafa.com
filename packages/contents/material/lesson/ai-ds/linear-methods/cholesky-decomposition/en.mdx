...
For positive definite matrices, there is a special property that makes decomposition much simpler. [LU decomposition](/material/lesson/ai-ds/linear-methods/lu-decomposition) can be performed without using a permutation matrix <InlineMath math="P" /> because Gaussian elimination can proceed without row swapping, and all pivot elements generated are guaranteed to be positive.
