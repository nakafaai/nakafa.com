...
Broadcasting adalah cara NumPy menangani operasi antara array dengan ukuran berbeda. NumPy otomatis mencari cara agar array dengan bentuk yang kompatibel bisa bekerja sama dalam perhitungan. Kamu bisa menjelajahi teknik broadcasting lanjutan di [panduan broadcasting NumPy](https://numpy.org/doc/stable/user/basics.broadcasting.html) ketika siap untuk skenario yang lebih kompleks.

Vektorisasi memungkinkan operasi pada seluruh array tanpa menulis loop, sehingga perhitungan menjadi lebih cepat karena elemen diproses secara bersamaan di level hardware.
...
1. **Aturan** <InlineMath math="1" />: Jika dimensi berbeda, tambahkan dimensi dengan ukuran <InlineMath math="1" /> dari sebelah kiri pada array yang dimensinya lebih kecil
2. **Aturan** <InlineMath math="2" />: Regangkan dimensi berukuran <InlineMath math="1" /> agar sesuai dengan nilai dimensi yang bersesuaian pada array lain
3. **Aturan** <InlineMath math="3" />: Jika bentuk tidak kompatibel, akan muncul error
...
Konsep sumbu (axis) dalam NumPy sangat penting. Untuk array dua dimensi, `axis=0` berarti operasi dilakukan sepanjang baris (menghasilkan nilai untuk setiap kolom), sedangkan `axis=1` berarti operasi dilakukan sepanjang kolom (menghasilkan nilai untuk setiap baris).
...
Fungsi `flatten()` dan `ravel()` keduanya mengubah array multidimensi menjadi array satu dimensi, tetapi dengan cara yang berbeda. Flatten membuat salinan baru dari data, sedangkan ravel mencoba membuat view yang lebih efisien dari segi memori.
...
Z-Transform adalah teknik standardisasi yang mengubah data agar memiliki rata-rata <InlineMath math="0" /> dan standar deviasi <InlineMath math="1" />. Teknik ini sangat berguna dalam machine learning untuk memastikan semua fitur memiliki skala yang sama.
