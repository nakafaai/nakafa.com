...
import {
  ...
  SubjectPlanTopicSchema,
} from "@repo/contents/_types/plan/schema";
import { Either, ParseResult, Schema } from "effect";
import { describe, expect, it } from "vitest";

describe("plan registry", () => {
  it("projects subject plans into localized navigation without route-local data files", () => {
  ...
  it("projects exercise plans into localized set navigation", () => {
  ...
  it("keeps plan keys and base routes unique", () => {
    const plans = listPlans();
    const keys = new Set(plans.map((plan) => plan.key));
    const baseRoutes = new Set(plans.map((plan) => plan.baseRoute));

    expect(keys.size).toBe(plans.length);
    expect(baseRoutes.size).toBe(plans.length);
    expect(listSubjectPlans()).not.toHaveLength(0);
    expect(listExercisePlans()).not.toHaveLength(0);
  });

  it("finds concrete plans by route without exposing the wrong plan kind", () => {
    expect(
      findSubjectPlan(
    ...
    expect(findSubjectPlan("exercises/high-school/snbt")).toBeNull();
    expect(findExercisePlan("subject/high-school/10/mathematics")).toBeNull();
  ...
  it("projects custom typed source chunks without optional descriptions", () => {
    const topic = defineSubjectPlanTopic({
      sections: [],
      slug: "custom-topic",
      translations: {
    ...
    const exercise = defineExercisePlan({
    ...
    expect(
      getSubjectMaterialList("subject/high-school/10/biology", "en", [plan])
    ).toEqual([
      {
  ...
  it("rejects invalid authored plan routes through the Effect Schema contract", () => {
    const result = Schema.decodeUnknownEither(SubjectPlanSourceSchema)({
  ...
  it("rejects invalid authored plan keys and slugs through the Effect Schema contract", () => {
    const invalidKey = Schema.decodeUnknownEither(SubjectPlanSourceSchema)({
      baseRoute: "subject/high-school/10/mathematics",
      category: "high-school",
      grade: "10",
      kind: "subject",
    ...
    const invalidSlug = Schema.decodeUnknownEither(SubjectPlanTopicSchema)({
      sections: [],
      slug: "Invalid Slug",
      translations: {
        en: { title: "Invalid" },
