...
The next model is different from the general-parts model above. It shows a more realistic **SARS-CoV-2** surface example. Use it to notice the outer envelope and many projecting surface proteins, not as the shape of every virus.

<SarsCov2VirionLab
  title={<>SARS-CoV-2 surface as a real example</>}
  description={
    <>
      Compare the outer envelope and surface proteins on one real virion.
    </>
  }
  labels={{
    ...
    items: [
      {
        tab: "Surface",
        caption: <>Surface proteins appear as many projections on the outside of the virion envelope.</>,
        ...
        callouts: [
          { id: "surface-protein", label: "surface proteins" },
          { id: "envelope", label: "outer envelope" },
        ],
      },
    ],
  }}
