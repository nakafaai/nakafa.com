    ...
    items: [
      {
        ...
        callouts: [
          { id: "genome-entry", label: "genome entry" },
          { id: "viral-parts", label: "viral parts" },
      ...
      {
        tab: "Lysogenic",
        caption: <>Host DNA carries the viral genome segment, and its copies travel with the cell when it divides.</>,
        callouts: [
...
The order matters because each stage addresses a different question. Attachment asks "which cell fits". Synthesis asks "where are viral parts made". Assembly asks "how does the new package form". Release asks "how do viruses reach other cells".

<Mermaid
  chart={`flowchart LR
    A["Adsorption when the virus attaches"] --> B["Penetration when the genome enters"]
    B --> C["Synthesis when genome and proteins are made"]
    C --> D["Assembly when new virions form"]
    D --> E["Lysis when virions exit"]`}
/>
