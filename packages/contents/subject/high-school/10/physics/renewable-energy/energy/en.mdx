...
The International System of Units (SI) unit for energy is the joule, with symbol <InlineMath math="\text{J}" />. Because energy can be calculated from force times displacement, a joule can be read as:
...
<Mermaid
  chart={`flowchart TD
    A["Sunlight"] --> B["Solar panel"]
    B --> C["Electrical energy"]
    C --> D["Lamp turns on"]
    E["Water at height"] --> F["Turbine"]
    F --> G["Generator"]
    G --> C
    H["Food"] --> I["Body"]
    I --> J["Muscles move"]`}/>
