...
<Mermaid
  chart={`flowchart TD
    ...
    A --> C["Mass"]
    A --> D["Time"]
    B --> E["Ruler, tape measure, vernier caliper, micrometer screw gauge"]
    C --> F["Balance"]
    D --> G["Clock or stopwatch"]
    E --> H["Result uses a length value and unit"]
    F --> I["Result uses a mass value and unit"]
    G --> J["Result uses a time value and unit"]`}
/>
...
Measurement always needs three things: the quantity we want to measure, the correct measuring tool, and the unit. When these three are clear, the result is easier to read, compare, and use in later scientific work.
