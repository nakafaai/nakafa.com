...
Official BIPM source for measurement units: [BIPM measurement units](https://www.bipm.org/en/measurement-units).

> SI is not just a list of abbreviations. SI is an agreement that <InlineMath math="1 \text{ m}" />, <InlineMath math="1 \text{ kg}" />, and <InlineMath math="1 \text{ s}" /> mean the same thing in different classrooms, laboratories, factories, and countries.
...
<Mermaid
  chart={`flowchart LR
    ...
    B --> C["Replace the prefix with a power-of-ten factor"]
    C --> D["Multiply the measured value"]
    D --> E["Write the target unit"]`}/>
...
Official BIPM source for SI prefixes: [BIPM SI prefixes](https://www.bipm.org/en/measurement-units/si-prefixes).
