export const metadata = {
  ...
  description: "Learn when to use mean, median, or mode in statistics and how outliers affect each measure in real data.",
...
- **Sensitivity Example:** If your test scores are <InlineMath math="7, 8, 9, 8, 10" /> (Mean is <InlineMath math="8.4" />), and then one additional score of <InlineMath math="1" /> (an outlier) is added, the new Mean becomes <InlineMath math="(7+8+9+8+10+1)/6 = 7.17" />. The Mean drops drastically due to the outlier.

### Median (Middle Value)

- **How to Calculate:** Sort the data, then find the value exactly in the middle position.
- **Property:** Only considers the middle position of the data. Therefore, the Median is **not sensitive to extreme values (outliers)**.
- **Outlier Example:** For data <InlineMath math="7, 8, 9, 8, 10" /> (sorted: <InlineMath math="7, 8, 8, 9, 10" />), the median is <InlineMath math="8" />. If a score of <InlineMath math="1" /> is added (sorted: <InlineMath math="1, 7, 8, 8, 9, 10" />), the median is <InlineMath math="(8+8)/2=8" />. The median hardly changes despite the outlier.

### Mode (Most Frequent Value)

- **How to Calculate:** Find the data value with the highest frequency.
- **Property:** Focuses on the most popular value. Like the Median, the Mode is **not sensitive to extreme values (outliers)**. Also, the Mode is the only measure of central tendency that can be used for **qualitative data** (non-numerical).
- **Outlier & Qualitative Example:** For data <InlineMath math="7, 8, 9, 8, 10" />, the mode is <InlineMath math="8" />. If a score of <InlineMath math="1" /> is added, the mode remains <InlineMath math="8" />. If the data is favorite colors (red, blue, red, green), the mode is red.
...
| **Median** | Quantitative (Numbers)     | Not Influenced    | When data **has outliers** or the distribution is skewed (not symmetrical). Provides a stable middle value. |
