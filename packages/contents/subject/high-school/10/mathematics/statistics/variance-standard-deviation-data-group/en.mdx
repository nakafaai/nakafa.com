...
How do we measure the spread of data presented in a grouped frequency table? For example, data on phone battery duration grouped into hour intervals (<InlineMath math="6\text{-}10" /> hours, <InlineMath math="11\text{-}15" /> hours, etc.).

Since we don't know the exact value of each data point within a class interval (e.g., in the <InlineMath math="11\text{-}15" /> hours class, we don't know if the duration was exactly <InlineMath math="11 \text{ hours}" />, <InlineMath math="12 \text{ hours}" />, or something else), we need to make an assumption.
