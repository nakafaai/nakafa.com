...
Dalam segitiga siku-siku, tangen sudut <InlineMath math="\theta" /> adalah perbandingan antara panjang sisi depan (opposite) dengan panjang sisi samping (adjacent). Ini sangat berbeda dengan sinus yang membandingkan sisi depan dengan hipotenusa, atau cosinus yang membandingkan sisi samping dengan hipotenusa.
...
Nilai tangen berubah sesuai dengan besar sudut. Sebagai contoh, <InlineMath math="\tan 30^\circ = 0,57 \text{ atau} \frac{1}{\sqrt{3}}" />.
...
2. Ukur atau ketahui sudut elevasi matahari <InlineMath math="\theta" />.
3. Gunakan rumus <InlineMath math="\tan \theta" /> untuk menghitung tinggi objek (sisi depan).
...
Dengan persamaan ini, jika kita mengetahui tinggi anak kecil (misalnya <InlineMath math="114 \text{ cm}" />), panjang bayangannya (<InlineMath math="200 \text{ cm}" />), dan tinggi remaja (<InlineMath math="148 \text{ cm}" />), kita bisa menghitung panjang bayangan remaja:
...
Misalnya, jika kita memiliki sudut elevasi matahari <InlineMath math="30^\circ" /> (<InlineMath math="\tan 30^\circ = 0,57" />) dan ingin menghitung panjang bayangan remaja dengan tinggi <InlineMath math="148 \text{ cm}" />:
...
<div className="mt-4">
  <Triangle
    title={<>Sudut <InlineMath math="30^\circ" /> dengan Tangen <InlineMath math="0,57" /></>}
    description={
      <>
