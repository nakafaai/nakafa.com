export const metadata = {
  ...
  description: "Understand series as sequential sums through handshake examples and see how adding sequence terms creates arithmetic and geometric patterns.",
...
Have you ever thought about how to sum the terms of a number sequence? For example, adding <InlineMath math="1 + 2 + 3 + \cdots" /> and so on? Well, this is what's called a **number series**.
...
- **If there are <InlineMath math="3 \text{ people}" />:** The first person shakes hands with <InlineMath math="2" /> others. The second person has already shaken hands with the first, so they only need to shake hands with the third person. The total is <InlineMath math="2 + 1 = 3 \text{ handshakes}" />.
