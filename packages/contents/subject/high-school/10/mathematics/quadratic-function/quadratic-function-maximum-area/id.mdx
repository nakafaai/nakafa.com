export const metadata = {
    ...
    description: "Maksimalkan luas dengan fungsi kuadrat melalui masalah nyata, solusi langkah demi langkah, dan interpretasi grafik. Pelajari teknik optimasi luas.",
...
Misalnya, Pak Tani punya pagar <InlineMath math="20 \text{ meter}" />. Dia mau buat kandang ayam bentuk persegi panjang. Berapa ukuran kandang biar luasnya paling besar?

1.  **Bikin Nama:** Misal panjangnya <InlineMath math="p \text{ meter}" /> dan lebarnya <InlineMath math="l \text{ meter}" />.
2.  **Hubungan Panjang Pagar:** Pagar itu kelilingnya.

    <BlockMath math="2p + 2l = 20" />

    Sederhanakan (semua suku dibagi <InlineMath math="2" />):
...
5.  **Cari Lebar Maksimum (l):** Pakai rumus <InlineMath math="x_p = -b / (2a)" /> (tapi <InlineMath math="x" /> nya kita ganti <InlineMath math="l" />):
    ...
    Jadi, lebarnya harus <InlineMath math="5 \text{ meter}" /> biar luasnya maksimal.
...
6.  **Cari Luas Maksimum (A):** Masukkan <InlineMath math="l = 5" /> ke rumus luas <InlineMath math="A(l) = 10l - l^2" />:
    ...
    Luas paling besarnya adalah <InlineMath math="25 \text{ m}^2" />.

7.  **Kesimpulan:** Biar luas kandang maksimal (<InlineMath math="25 \text{ m}^2" />), lebarnya <InlineMath math="l=5 \text{ meter}" />. Panjangnya <InlineMath math="p = 10 - l = 10 - 5 = 5 \text{ meter}" />. Ukurannya harus <InlineMath math="5 \text{ m} \times 5 \text{ m}" /> (ternyata bentuknya persegi!).
...
- Untung maksimalnya: <InlineMath math="K(25) = 100(25) - 2(25^2) = 2500 - 2(625) = 2500 - 1250 = 1250" /> ribu rupiah (atau <InlineMath math="\text{Rp }1.250" />.<InlineMath math="000" />).
...
Roket mainan terbang! Tingginya setelah <InlineMath math="t \text{ detik}" /> adalah <InlineMath math="h(t) = 40t - 5t^2 \text{ meter}" />. Kapan roket paling tinggi dan berapa tingginya?

- Fungsi kuadratnya: <InlineMath math="h(t) = -5t^2 + 40t" />. <InlineMath math="a = -5, b = 40" />.
- Waktu paling tinggi: <InlineMath math="t_p = -b / (2a) = -40 / (2 \times -5) = -40 / -10 = 4 \text{ detik}" />.
- Tinggi paling tinggi: <InlineMath math="h(4) = 40(4) - 5(4^2) = 160 - 5(16) = 160 - 80 = 80 \text{ meter}" />.

## Latihan

Sebuah persegi panjang punya keliling <InlineMath math="60 \text{ cm}" />. Tentukan ukuran panjang dan lebarnya agar luasnya maksimum, dan hitung luas maksimumnya!
...
Jadi, agar luasnya maksimum (<InlineMath math="L_{maks} = 225 \text{ cm}^2" />), ukurannya harus <InlineMath math="15 \text{ cm}" /> x <InlineMath math="15 \text{ cm}" /> (bentuk persegi lagi!).
