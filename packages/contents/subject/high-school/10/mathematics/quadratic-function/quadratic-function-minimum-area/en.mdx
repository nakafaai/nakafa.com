export const metadata = {
    ...
    description: "Minimize areas and costs using quadratic functions with practical examples, efficient solutions, and business applications. Learn cost optimization techniques.",
...
Let's say we have a <InlineMath math="40 \text{ cm}" /> long wire. We want to cut this wire into two parts. The first part will be formed into a square, and the second part will also be formed into a square. What should the cuts be so that the _total area_ of both squares is as small as possible (minimum)?

1.  **Name Things:** Let's say the side of the first square is <InlineMath math="x \text{ cm}" />, and the side of the second square is <InlineMath math="y \text{ cm}" />.
2.  **Wire Length Relationship:**

    - Wire for the first square: Perimeter is <InlineMath math="4x \text{ cm}" />.
    - Wire for the second square: Perimeter is <InlineMath math="4y \text{ cm}" />.
    - Total wire length:

      <BlockMath math="4x + 4y = 40" />

      Simplify (divide by <InlineMath math="4" />):
...
5.  **Find Side x for Minimum Area:** Use the formula <InlineMath math="x_p = -b / (2a)" />:
    ...
    So, the side of the first square must be <InlineMath math="5 \text{ cm}" /> for the sum of areas to be minimal.

6.  **Find Side y and Minimum Area:**

    - Side of the second square: <InlineMath math="y = 10 - x = 10 - 5 = 5 \text{ cm}" />.
...
The production cost of <InlineMath math="x \text{ units}" /> of goods (in thousands of rupiah) is <InlineMath math="C(x) = 3x^2 - 60x + 500" />. How many units should be produced to minimize the cost?

- Cost function: <InlineMath math="C(x) = 3x^2 - 60x + 500" /> (<InlineMath math="a=3, b=-60" />). <InlineMath math="a>0" />, so there is a minimum.
- Number of units for minimum cost: <InlineMath math="x_p = -b / (2a) = -(-60) / (2 \times 3) = 60 / 6 = 10 \text{ units}" />.
- Minimum cost: <InlineMath math="C(10) = 3(10)^2 - 60(10) + 500 = 3(100) - 600 + 500 = 300 - 600 + 500 = 200" /> thousand rupiah (or <InlineMath math="\text{Rp }200{,}000" />).

## Exercise

The sum of two positive numbers is <InlineMath math="16" />. Determine these two numbers so that the sum of their squares is minimum, and calculate the minimum sum of squares!
...
Therefore, the two numbers are <InlineMath math="8" /> and <InlineMath math="8" /> for the sum of their squares to be minimum (which is <InlineMath math="128" />).
