...
Unlike the floor and ceiling functions, the unit step function has a value of <InlineMath math="0" /> for <InlineMath math="x < 0" /> and <InlineMath math="1" /> for <InlineMath math="x \geq 0" />.
...
5. **Calculating the number of elevator operations:**
   ...
   If there are <InlineMath math="n \text{ people}" /> and the elevator can accommodate a maximum of <InlineMath math="8 \text{ people}" />, then the number of elevator operations required is:

   <BlockMath math="f(n) = \lceil \frac{n}{8} \rceil" />

   The ceiling function is used because if there are remaining people (less than <InlineMath math="8" />), one additional elevator operation is still required.
