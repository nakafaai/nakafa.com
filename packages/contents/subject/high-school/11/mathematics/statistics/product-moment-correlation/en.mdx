...
If the points still show an upward trend but are more scattered like this, the <InlineMath math="r" /> value is positive but smaller (closer to <InlineMath math="0" />).
...
The Pearson correlation coefficient (<InlineMath math="r" />) essentially measures how **synchronously** two variables (<InlineMath math="X" /> and <InlineMath math="Y" />) move relative to their own variations.

**Imagine this:**

1.  **Individual Variation:**

    Each variable (<InlineMath math="X" /> and <InlineMath math="Y" />) has its own variability. Some values fluctuate a lot (large variation), while others are stable (small variation). This is measured by <InlineMath math="SS_{xx}" /> for X and <InlineMath math="SS_{yy}" /> for Y (formulas below).

2.  **Joint Variation (Covariance):**

    We also need to know how <InlineMath math="X" /> and <InlineMath math="Y" /> vary _together_. When X increases, does Y also tend to increase? Or decrease? This measure of joint variation is called **covariance**, calculated using <InlineMath math="SS_{xy}" />.

    - If <InlineMath math="SS_{xy}" /> is large and positive: <InlineMath math="X" /> and <InlineMath math="Y" /> often move in the same direction.
    - If <InlineMath math="SS_{xy}" /> is large and negative: <InlineMath math="X" /> and <InlineMath math="Y" /> often move in opposite directions.
...
The result of this division is **<InlineMath math="r" />**, the Pearson Correlation Coefficient. Because it's standardized, its value will always be between <InlineMath math="-1" /> and <InlineMath math="+1" />, regardless of the original data units. This allows us to compare the strength of linear relationships between different pairs of variables.

So, the value of <InlineMath math="r" /> is determined by comparing how strongly <InlineMath math="X" /> and <InlineMath math="Y" /> move together relative to how much they move individually.
