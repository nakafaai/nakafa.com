...
One of its main uses is for **prediction**. For example, if a new student studies for <InlineMath math="7 \text{ hours}" />, we can use this regression line to estimate what their exam score might be, even though we don't have exact data for <InlineMath math="7 \text{ hours}" />.
