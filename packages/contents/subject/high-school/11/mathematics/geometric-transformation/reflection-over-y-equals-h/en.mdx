...
export const metadata = {
  ...
  description: "Understand horizontal line reflections with worked examples, visual guides, and point-image rules for geometry problems.",
...
The vertical distance from the original point to the mirror line is equal to the vertical distance from the image point to the mirror line. The <InlineMath math="x" />-coordinate of the point does not change.
...
Thus, the image of point <InlineMath math="P(x, y)" /> is <InlineMath math="P'(x, 2h - y)" />. Note that the <InlineMath math="x" />-coordinate remains the same, while the <InlineMath math="y" />-coordinate changes based on its distance from the line <InlineMath math="y=h" />.
...
3.  Given the image <InlineMath math="R'(-2,5)" /> and the mirror line <InlineMath math="y=4" />. So <InlineMath math="x'=-2, y'=5, h=4" />.
    ...
    Thus, the coordinates of point <InlineMath math="R" /> are <InlineMath math="R(-2,3)" />.
