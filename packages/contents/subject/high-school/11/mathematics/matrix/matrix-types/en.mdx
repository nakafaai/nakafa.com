export const metadata = {
  title: "Matrix Types",
  description: "Learn the main matrix types: row, column, square, triangular, diagonal, identity, zero, and symmetric matrices, with definitions and examples.",
...
If <InlineMath math="\text{number of rows} = \text{number of columns} = n" />, then the matrix is of order <InlineMath math="n \times n" />.
...
Matrix <InlineMath math="C" /> above has <InlineMath math="2" /> rows and <InlineMath math="3" /> columns, so its order is <InlineMath math="2 \times 3" />. Since the number of rows is not equal to the number of columns (<InlineMath math="2 \neq 3" />), matrix <InlineMath math="C" /> is a rectangular matrix.
...
An identity matrix (denoted by <InlineMath math="I" /> or <InlineMath math="I_n" />) is a diagonal matrix where all elements on the main diagonal are <InlineMath math="1" />.
