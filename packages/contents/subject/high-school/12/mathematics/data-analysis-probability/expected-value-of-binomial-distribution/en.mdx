export const metadata = {
    ...
    description: "Understand expected value in binomial distributions and calculate mean outcomes in repeated trials using E(X)=np.",
...
A fair die is rolled <InlineMath math="7\text{ times}" />. What is the expected value for getting a five on the die from all the rolls?
...
- Fixed number of trials: <InlineMath math="7\text{ times}" />
...
Expected value <InlineMath math="1.167" /> doesn't mean you will get <InlineMath math="1.167 \text{ times}" /> the number <InlineMath math="5" /> in one experiment (because that's impossible). What it means is: if you repeat this experiment of rolling a die <InlineMath math="7 \text{ times}" /> **hundreds or thousands of times**, then on average you will get the number <InlineMath math="5" /> about <InlineMath math="1.167 \text{ times}" /> per set of <InlineMath math="7" /> rolls.

In practice, in one set of <InlineMath math="7" /> rolls, you might get the number <InlineMath math="5" /> as many as <InlineMath math="0" />, <InlineMath math="1" />, <InlineMath math="2" />, or even <InlineMath math="3\text{ times}" />. But when averaged over the long term, the result will approach <InlineMath math="1.167" />.
