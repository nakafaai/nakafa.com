...
Horizontal translation is a geometric transformation that shifts the graph of a function left or right along the <InlineMath math="x" />-axis without changing the shape of the graph. Imagine sliding an object horizontally on a table, its shape remains the same, only its position changes.
...
Where:
- If <InlineMath math="h > 0" />, the graph shifts to the **right** by <InlineMath math="h \text{ units}" />
- If <InlineMath math="h < 0" />, the graph shifts to the **left** by <InlineMath math="|h| \text{ units}" />
...
- Function <InlineMath math="g(x) = (x - 3)^2" /> shifts right by <InlineMath math="3 \text{ units}" /> with vertex at <InlineMath math="(3, 0)" />
- Function <InlineMath math="h(x) = (x + 2)^2" /> shifts left by <InlineMath math="2 \text{ units}" /> with vertex at <InlineMath math="(-2, 0)" />
...
Notice that:
- All lines have the same slope of <InlineMath math="2" />
- Function <InlineMath math="g(x) = 2(x - 4) + 1" /> shifts right by <InlineMath math="4 \text{ units}" />
- Function <InlineMath math="h(x) = 2(x + 3) + 1" /> shifts left by <InlineMath math="3 \text{ units}" />
...
- **Domain**: Shifts by <InlineMath math="h \text{ units}" />
...
- The <InlineMath math="y" />-intercept changes due to horizontal shift
- Function <InlineMath math="g(x) = 2^{x-2}" /> shifts right by <InlineMath math="2 \text{ units}" />
- Function <InlineMath math="h(x) = 2^{x+1}" /> shifts left by <InlineMath math="1 \text{ unit}" />
...
2. If the graph of function <InlineMath math="g(x) = \sqrt{x}" /> is translated horizontally to the left by <InlineMath math="4 \text{ units}" />, determine:
