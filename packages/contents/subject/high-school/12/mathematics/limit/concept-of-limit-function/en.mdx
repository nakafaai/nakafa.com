...
To understand limits more concretely, let's see how function values change when the variable approaches a certain point. Suppose we have function <InlineMath math="f(x) = x + 2" /> and want to see what happens when <InlineMath math="x" /> approaches <InlineMath math="3" />.
...
From the table above, we can see that when <InlineMath math="x" /> approaches <InlineMath math="3" /> from the left (values <InlineMath math="x < 3" />) and from the right (values <InlineMath math="x > 3" />), the value of <InlineMath math="f(x)" /> approaches <InlineMath math="5" />. **This approaching value is called the limit**.
...
Since we are calculating the limit when <InlineMath math="x" /> **approaches** <InlineMath math="2" /> (not **equals** <InlineMath math="2" />), then <InlineMath math="x \neq 2" /> and we can cancel <InlineMath math="(x - 2)" />:
...
2. **Solution:**
   ...
   Since <InlineMath math="x" /> approaches <InlineMath math="1" /> (not equal to <InlineMath math="1" />), then <InlineMath math="x \neq 1" /> and we can cancel <InlineMath math="(x - 1)" />:
...
4. **Solution:**
   ...
   **Right limit** (when <InlineMath math="x" /> approaches <InlineMath math="2" /> from the right, so <InlineMath math="x \geq 2" />):
