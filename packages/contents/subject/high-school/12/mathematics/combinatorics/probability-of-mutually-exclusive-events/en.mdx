...
Since there is an intersection (number <InlineMath math="5" />), the two events are **not mutually exclusive**. Let's use an example that is truly mutually exclusive.
...
To add <InlineMath math="\frac{1}{9} + \frac{1}{6}" />, we need to find the LCM of <InlineMath math="9" /> and <InlineMath math="6" />.
...
- **Double-check** whether the final result makes sense (probability must be between <InlineMath math="0" /> and <InlineMath math="1" />)
...
- Event <InlineMath math="B" />: Students who are smart (<InlineMath math="\text{score} > 80" />)
