...
2.  **Power Rule**: This is a useful rule for functions of the form <InlineMath math="f(x) = ax^n" />. The method is simple: multiply the exponent <InlineMath math="n" /> by the coefficient <InlineMath math="a" />, then subtract one from the exponent.
