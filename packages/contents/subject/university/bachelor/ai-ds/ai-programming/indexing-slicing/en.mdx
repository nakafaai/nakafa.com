export const metadata = {
    ...
    description: "Learn Python indexing and slicing techniques to access string and list elements. Learn positive, negative indices and stride usage.",
...
Python uses a numbering system that starts from <InlineMath math="0" />, not <InlineMath math="1" />. So the first element has index <InlineMath math="0" />, the second element has index <InlineMath math="1" />, and so on. Additionally, Python also supports negative indices that count backwards from the end.
...
Negative stride is very useful for reversing element order. When stride is <InlineMath math="-1" />, Python will take all elements from end to start.
