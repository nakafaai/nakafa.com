...
# range(stop) - dari <InlineMath math="0" /> sampai stop-1
for i in range(4):
    print(i)
# Output: 0, 1, 2, 3
...
Range mengikuti formula aritmatika dimana setiap elemen ke-i adalah `start + i * step` untuk semua `i` dari <InlineMath math="0" /> sampai `n-1`.
