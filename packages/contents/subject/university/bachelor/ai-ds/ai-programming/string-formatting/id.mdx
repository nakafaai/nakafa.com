...
<Mermaid
  title="Opsi Perataan"
  description="Bandingkan rata kiri, kanan, dan tengah saat teks yang sama ditempatkan dalam lebar tetap."
  chart={`
    flowchart TD
...
|--------|------|----------|---------------|
| `<` | Rata kiri | Teks di kiri, padding di kanan | String, objek |
| `>` | Rata kanan | Teks di kanan, padding di kiri | Angka |
| `^` | Tengah | Teks di tengah, padding merata | - |

### Karakter Pengisi dan Format Lanjutan

Karakter pengisi menentukan karakter untuk mengisi ruang kosong. Default-nya adalah spasi, tetapi bisa diganti dengan karakter apa pun kecuali `{` dan `}`.
...
<Mermaid
  title="Metode Referensi Argumen"
...
Presisi mengontrol jumlah digit desimal atau digit signifikan dalam tampilan angka.
...
F-strings tersedia sejak Python `3.6+` dan menyediakan sintaks paling sederhana dengan evaluasi ekspresi langsung. Lebih cepat dan mudah dibaca dibandingkan `format()`.
...
<Mermaid
  title="Pola Lengkap Spesifikasi Format"
  description="Baca urutan fill, align, width, precision, dan type agar mini-language format tidak terlihat acak."
  chart={`
    flowchart TD
