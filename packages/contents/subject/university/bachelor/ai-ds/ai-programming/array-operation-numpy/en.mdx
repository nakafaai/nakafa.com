export const metadata = {
    ...
    description: "Learn NumPy array operations: broadcasting, vectorization, arithmetic, statistics, and shape manipulation with practical examples for data science.",
...
Broadcasting is NumPy's way of handling operations between arrays of different sizes. It automatically figures out how to make arrays with compatible shapes work together in calculations. You can explore more advanced broadcasting techniques in the [NumPy broadcasting guide](https://numpy.org/doc/stable/user/basics.broadcasting.html) when you're ready for complex scenarios.

Vectorization allows operations on entire arrays without writing loops, making calculations faster by processing elements simultaneously at the hardware level.
...
1. **Rule** <InlineMath math="1" />: If dimensions differ, add dimensions of size <InlineMath math="1" /> from the left on the array with smaller dimensions
2. **Rule** <InlineMath math="2" />: Stretch dimensions of size <InlineMath math="1" /> to match the corresponding dimension values of the other array
3. **Rule** <InlineMath math="3" />: If shapes are incompatible, an error will occur
...
The concept of axes in NumPy is very important. For two-dimensional arrays, `axis=0` means operations are performed along rows (producing values for each column), while `axis=1` means operations are performed along columns (producing values for each row).
...
Both `flatten()` and `ravel()` functions convert multidimensional arrays to one-dimensional arrays, but in different ways. Flatten creates a new copy of the data, while ravel tries to create a more memory-efficient view.
...
Z-Transform is a standardization technique that transforms data to have a mean of <InlineMath math="0" /> and standard deviation of <InlineMath math="1" />. This technique is very useful in machine learning to ensure all features have the same scale.
...
For more details about NumPy array operations, you can visit the [official NumPy documentation](https://numpy.org/doc/stable/user/basics.html), which provides guides and practical examples.
