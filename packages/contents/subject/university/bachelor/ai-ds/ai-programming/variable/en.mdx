export const metadata = {
    title: "Variables",
    description: "Learn Python variable concepts, naming rules, reserved keywords, and how memory works with practical examples.",
...
When you write `x = 2`, Python does three important things. First, Python creates an object with the value <InlineMath math="2" /> in memory. Second, Python stores that object at a specific memory address. Third, Python connects the variable name `x` to that memory address.
...
<Mermaid
  title="How Variables and Memory Work"
...
Variable names in Python can consist of three main components. First are letters, both lowercase (`a`, `b`, `c`, ..., `z`) and uppercase (`A`, `B`, `C`, ..., `Z`). Second are numbers (<InlineMath math="0, 1, 2, \dots, 9" />). Third is the underscore character (_).
