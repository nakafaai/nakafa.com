export const metadata = {
    ...
    description: "Learn how to use Python's math module for sqrt(), round(), abs(), the pi constant, importing modules, and practical calculations.",
...
The `round()` function rounds floating-point numbers to the nearest integer using the Banker's rounding system. In this system, numbers that are exactly in the middle (like <InlineMath math="0.5" />) are rounded to the nearest even number.
...
Note that `round(4.5)` produces <InlineMath math="4" />, not <InlineMath math="5" />. This is because Python uses Banker's rounding which rounds to the nearest even number for cases exactly in the middle. The `round()` function also accepts a second parameter to determine the number of decimal digits or rounding position.
