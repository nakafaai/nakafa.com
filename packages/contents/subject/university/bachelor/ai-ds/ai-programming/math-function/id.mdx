export const metadata = {
    ...
    description: "Pelajari cara menggunakan modul math Python untuk sqrt(), round(), abs(), konstanta pi, import modul, dan perhitungan praktis.",
...
Fungsi `round()` membulatkan bilangan floating-point ke bilangan bulat terdekat menggunakan sistem Banker's rounding. Dalam sistem ini, angka yang berada tepat di tengah (seperti <InlineMath math="0.5" />) dibulatkan ke bilangan genap terdekat.
...
Perhatikan bahwa `round(4.5)` menghasilkan <InlineMath math="4" />, bukan <InlineMath math="5" />. Ini karena Python menggunakan Banker's rounding yang membulatkan ke bilangan genap terdekat untuk kasus tepat di tengah. Fungsi `round()` juga menerima parameter kedua untuk menentukan jumlah digit desimal atau posisi pembulatan.
