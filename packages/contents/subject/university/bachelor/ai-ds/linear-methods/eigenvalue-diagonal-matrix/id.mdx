...
**Bukti**: Karena <InlineMath math="\chi_A(t) = \det(A - t \cdot I) = (a_{11} - t) \cdots (a_{nn} - t)" /> dengan akar-akar <InlineMath math="a_{11}, \ldots, a_{nn}" />.
...
Ada hubungan fundamental antara nilai eigen dengan determinan dan jejak matriks. Jika polinomial karakteristik <InlineMath math="\chi_A(t)" /> dapat difaktorkan secara linear di <InlineMath math="\mathbb{K}" />, artinya matriks <InlineMath math="A" /> memiliki <InlineMath math="n" /> nilai eigen <InlineMath math="\lambda_1, \ldots, \lambda_n \in \mathbb{K}" />, maka:
...
**Determinan adalah hasil kali** semua nilai eigen, sedangkan **jejak adalah jumlah** semua nilai eigen.
