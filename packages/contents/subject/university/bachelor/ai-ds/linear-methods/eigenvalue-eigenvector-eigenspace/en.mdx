...
Why is this true? <InlineMath math="A \cdot (t \cdot v) = t \cdot (A \cdot v) = t \cdot (\lambda \cdot v) = \lambda \cdot (t \cdot v)" />
...
There's a very important result about linear independence of eigenvectors. Let <InlineMath math="A \in \mathbb{K}^{n \times n}" /> and <InlineMath math="\lambda_1, \ldots, \lambda_k \in \mathbb{K}" /> be pairwise distinct eigenvalues of <InlineMath math="A" />, that is <InlineMath math="\lambda_i \neq \lambda_j" /> for <InlineMath math="i \neq j" /> with <InlineMath math="i, j \in \{1, \ldots, k\}" />. Then the corresponding eigenvectors <InlineMath math="v_1, \ldots, v_k \in \mathbb{K}^n" /> are linearly independent.
...
Now, let's look at an interesting relationship between invertibility and eigenvalues. Matrix <InlineMath math="A \in \mathbb{K}^{n \times n}" /> is invertible if and only if all eigenvalues <InlineMath math="\lambda \in \mathbb{K}" /> of <InlineMath math="A" /> satisfy <InlineMath math="\lambda \neq 0" />.

Why is this true? <InlineMath math="A" /> is invertible if and only if <InlineMath math="\text{Rank} A = n" />, which means <InlineMath math="\text{Eig}_A(0) = \ker A = \{0\}" />, so <InlineMath math="\lambda = 0" /> is not an eigenvalue of <InlineMath math="A" />.
...
Why is this true? From <InlineMath math="A \cdot v = \lambda \cdot v" />, by multiplying <InlineMath math="A^{-1}" /> and <InlineMath math="\frac{1}{\lambda}" />, we get <InlineMath math="\frac{1}{\lambda} \cdot v = A^{-1} \cdot v" />.
...
7. All eigenvalues of <InlineMath math="A" /> are not equal to <InlineMath math="0" />
