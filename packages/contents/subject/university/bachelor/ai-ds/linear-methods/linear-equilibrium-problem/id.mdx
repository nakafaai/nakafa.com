...
Bayangkan kamu sedang mencoba mencocokkan puzzle yang tidak sempurna. Begitulah situasi yang kita hadapi dalam masalah keseimbangan linear. Kita memiliki matriks <InlineMath math="A \in \mathbb{R}^{m \times n}" /> dan vektor <InlineMath math="b \in \mathbb{R}^m" />, tetapi sistem persamaan <InlineMath math="A \cdot x = b" /> mungkin tidak memiliki solusi yang tepat.
