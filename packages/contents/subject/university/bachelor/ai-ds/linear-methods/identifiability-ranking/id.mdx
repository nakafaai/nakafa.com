...
Perbedaan antara kedua pendekatan ini seperti membandingkan pengukuran dengan penggaris biasa versus pengukuran dengan alat presisi tinggi. Dekomposisi nilai singular memberikan informasi yang lebih detail dan robust tentang struktur numerik matriks, terutama untuk kasus mendekati singular.
...
### Kondisi Rank-Deficient

Istilah "rank-deficient" (defisiensi peringkat) merujuk pada kondisi ketika matriks tidak memiliki peringkat penuh. Artinya, <InlineMath math="\text{Peringkat}(A) < \min(m, n)" />. Dalam konteks ini, beberapa baris atau kolom matriks saling bergantung secara linear.

Ketika matriks mengalami rank-deficient, beberapa nilai singular menjadi nol atau sangat mendekati nol
