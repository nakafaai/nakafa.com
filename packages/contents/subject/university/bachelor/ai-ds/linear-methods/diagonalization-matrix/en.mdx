...
which has dimension <InlineMath math="1" />. Since there are no other eigenvalues and eigenvectors, and there is no basis of <InlineMath math="\mathbb{K}^2" /> consisting of eigenvectors of <InlineMath math="A" />, then <InlineMath math="A" /> is not diagonalizable.
