...
Di sini rentang dari vektor <InlineMath math="\begin{pmatrix} 1 \\ 0 \end{pmatrix}" /> adalah himpunan semua kelipatan skalar dari vektor tersebut, yang memiliki dimensi <InlineMath math="1" />. Karena tidak ada nilai eigen dan vektor eigen lain, dan tidak terdapat basis <InlineMath math="\mathbb{K}^2" /> dari vektor eigen <InlineMath math="A" />, maka <InlineMath math="A" /> tidak dapat didiagonalisasi.
