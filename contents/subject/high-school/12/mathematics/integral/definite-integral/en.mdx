...
We can factor constants out of the sigma notation. Remember that <InlineMath math="n" /> is treated as a constant in the sum from <InlineMath math="i=1" /> to <InlineMath math="n" />.

<BlockMath math="\frac{49}{n^2} \sum_{i=1}^{n} i" />

Using the series summation formula, we can replace <InlineMath math="\sum_{i=1}^{n} i" />.
