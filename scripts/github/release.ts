...
function releaseError(message: string, cause: unknown) {
  return new GithubActionReleaseError({ cause, message });
}

/** Returns one consistent latest-release review for each upstream repository. */
export const githubActionReleaseReviews = Effect.fn("GithubRelease.reviews")(
  function* () {
...
export const fetchLatestGithubActionTag = Effect.fn("GithubRelease.fetch")(
  function* (
    ...
    return yield* client.get(url, { headers }).pipe(
      ...
      Effect.mapError((cause) =>
        releaseError(
          `Unable to read the latest ${review.repository} release.`,
          cause
        )
      )
    );
