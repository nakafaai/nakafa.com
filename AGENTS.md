...
- `scripts/check/effect.ts` enforces the backend rule inside `packages/backend`: no raw `try` statement and no `typeof ... === "object"` narrowing. Narrow unknown input with `Predicate` or `Schema` and model expected failure with a tagged Effect error.
