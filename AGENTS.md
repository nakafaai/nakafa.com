...
- `scripts/check/effect.ts` enforces this in every authored module under `apps`, `packages`, and `scripts`: no raw `try` statement and no `typeof ... === "object"` narrowing. Narrow unknown input with `Predicate` or `Schema` and model expected failure with a tagged Effect error.
