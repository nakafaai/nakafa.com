...
- Cookieless baseline is stateless server hash. The consent gate and browser
  SDK posture landed with #617 and are unchanged by this runbook.
...
- This runbook only. No SDK init, consent gate, proxy, or instrumentation
  file is touched, so it does not overlap the merged #617 change
  (`fix/analytics-always-on-baseline`).
