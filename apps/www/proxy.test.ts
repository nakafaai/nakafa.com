...
describe("proxy", () => {
  ...
  ])("rewrites markdown requests with an %s", async (_kind, path, init, expected) => {
    const response = await requestProxy(path, init);
  ...
  ])("returns a hard 404 for missing HTML route %s", async (path, rewrite, projectedPath) => {
    if (projectedPath) {
  ...
  ])("delegates %s/%s to the locale middleware", async (locale, path, publicPath, kind) => {
  ...
  it("routes active material additions and rejects owned tombstones", async () => {
    ...
    runtimeMocks.readActive
      .mockReturnValueOnce(Effect.succeed({ kind: "found" }))
