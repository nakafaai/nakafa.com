...
export const config = {
  matcher: [
    // Run middleware on all routes except static assets and API routes (but allow XML files for sitemap handling)
    "/((?!_next/static|_pagefind|fonts|open-graph|api|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp|js|css|webmanifest|txt)$).*)",
  ],
};
