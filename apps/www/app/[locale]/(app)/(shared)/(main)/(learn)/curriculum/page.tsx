...
import { getGradeIcon } from "@/app/[locale]/(app)/(shared)/(main)/(learn)/curriculum/icons";
import { HeaderContent } from "@/components/shared/header-content";
import { LayoutContent } from "@/components/shared/layout-content";
import { applyContentRuntimeCache } from "@/lib/content/cache";
import { getRuntimeCurriculumGrades } from "@/lib/content/navigation";
...
export async function generateMetadata({
  ...
  params: PageProps<"/[locale]/curriculum">["params"];
}): Promise<Metadata> {
  ...
  const socialMetadata = getSocialMetadata({
    ...
    locale,
    path,
    image: getOgUrl(locale, "/curriculum"),
  });
...
export default function Page(props: PageProps<"/[locale]/curriculum">) {
  const { params } = props;
...
/** Reads curriculum grade listings inside a Next Cache Components boundary. */
async function getCachedCurriculumGrades(locale: Locale) {
  "use cache";
  applyContentRuntimeCache();

  return Effect.runPromise(getRuntimeCurriculumGrades(locale));
}

/** Renders the curriculum root grouped by Convex-backed category and grade rows. */
async function PageContent({ locale }: { locale: Locale }) {
  const allGrades = await getCachedCurriculumGrades(locale);
