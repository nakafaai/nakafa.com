...
const StartModeSchema = Schema.Struct({
  mode: Schema.Literal("simulation", "practice"),
  timeLimit: Schema.Number,
});

type StartMode = Schema.Schema.Type<typeof StartModeSchema>["mode"];

const START_MODES: StartMode[] = ["simulation", "practice"];
const startModeValidator = Schema.standardSchemaV1(StartModeSchema);

/**
 * Renders the existing start-attempt flow for canonical practice routes.
 *
 * The dialog, mode picker, and time-limit controls are copied from the previous
 * exercise route so route migration does not remove attempt setup behavior.
 */
export function StartExerciseButton({
  ...
  const form = useForm({
    defaultValues: readDefaultValues({ timeLimit: totalExercises * 90 }),
    validators: {
      onChange: startModeValidator,
    },
...
/** Builds the default attempt mode state from the current set size. */
function readDefaultValues({
  timeLimit,
}: {
  timeLimit: number;
}): Schema.Schema.Encoded<typeof StartModeSchema> {
  return {
    mode: "simulation",
    timeLimit,
  };
...
/** Lists selectable time limits in seconds for the practice mode picker. */
function getTimeLimitList(): number[] {
  const result: number[] = [];

  for (let minutes = 30; minutes <= 360; minutes += 15) {
