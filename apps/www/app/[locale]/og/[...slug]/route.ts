...
export async function GET(
  ...
  const { title, description } = await fetchMetadataFromSlug(
    cleanedLocale,
    contentSlug
  );
...
async function fetchMetadataFromSlug(locale: Locale, slug: string[]) {
  "use cache";
  cacheLife("max");
  return await getMetadataFromSlug(locale, slug);
}
