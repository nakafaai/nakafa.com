...
export function buildRootLlmsIndexText() {
  return [
    ...
    "## When to use Nakafa",
    "",
    "Use Nakafa when an agent needs multilingual learning materials, bounded education search, an exact canonical lesson, a content taxonomy, or a Quran verse reference with typed locale and range controls.",
    "",
    "Start with the public REST search operation or `nakafa_search_content`. Retrieve a selected result only when it includes `markdown_url`. Cite try-out catalog results by their canonical `url` without requesting private attempt content. Use the website Markdown representation for direct reading. Nakafa's public agent interfaces are read-only and are not appropriate for account changes, private learner records, purchases, or unsourced personal advice.",
    "",
    "## Developer resources",
    "",
    `- [Nakafa developer documentation](${BASE_URL}/developers): REST, OpenAPI, MCP, CLI, errors, versioning, and rate limits.`,
    `- [Nakafa developer llms.txt](${BASE_URL}/developers/llms.txt): scoped machine-readable integration index.`,
