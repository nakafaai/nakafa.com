...
describe("llms content entries", () => {
  it.effect("builds sorted page entries from signed inventories", () =>
    Effect.gen(function* () {
  ...
  it.effect("uses one exact signed article partition", () =>
    Effect.gen(function* () {
      mockReadPublishedArticleBuckets.mockReturnValue(
        Effect.succeed({
          activeReleaseId: activeArticleReleaseId,
      ...
      expect(
        yield* getContentPageLlmsEntries({
          locale: "en",
          page: 0,
          section: "articles",
        })
  ...
  it.effect(
    "uses published material partitions after ownership activates",
    () =>
      Effect.gen(function* () {
        mockReadMaterialInventory.mockReturnValue(
          Effect.succeed({
  ...
  it.effect("distinguishes an empty signed page from a missing page", () =>
    Effect.gen(function* () {
  ...
  it.effect(
    "rejects absent or changed published partitions without source fallback",
    () =>
      Effect.gen(function* () {
        mockReadPublishedArticleBuckets.mockReturnValue(
          Effect.succeed({
