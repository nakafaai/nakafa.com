...
describe("published content artifact execution", () => {
  it.each([
    ...
    "compiles and executes the actual %s Function Concept source",
    async (locale, heading, machineTitle, inputLabel) => {
  ...
  it.each([
    ...
    "compiles and executes the actual %s Atom Shell source",
    async (locale, heading, labTitle) => {
      const rawMdx = await readImportFreeMdx(
        atomShellRoot,
        atomShellImport,
        locale
