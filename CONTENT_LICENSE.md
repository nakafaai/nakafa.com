# Nakafa Content License 1.0

Effective date: April 24, 2026

Copyright (c) 2026 PT NAKAFA TEKNO KREATIF. All rights reserved.

Nakafa educational content is public for learning and review. It is not public
domain, open content, or free material for republishing, scraping, rebranding,
commercial use, or AI training.

## 1. Definitions

`Nakafa content` means the non-code materials covered by this license.

`You` means the individual or entity receiving or using Nakafa content.

`Personal use` means use by one individual for that individual's own learning.
Personal use does not include use for an employer, client, school, organization,
product, service, team, or business purpose.

`Written permission` means a separate written agreement signed by PT NAKAFA
TEKNO KREATIF, or a written approval sent from an official Nakafa email address.

## 2. Scope

This license applies to Nakafa non-code materials in this repository and in the
Nakafa services, unless a file or page says that a different license applies.

Covered materials include:

- Lessons, articles, exercises, explanations, tryout questions, answer keys,
  hints, summaries, and translations.
- MDX, Markdown, JSON, TypeScript data files, metadata, prompts, examples,
  diagrams, charts, images, video, audio, downloadable files, and page copy.
- The structure, selection, arrangement, and compilation of Nakafa content.

Third-party content keeps its own license or legal status. This license does not
grant rights to third-party materials that Nakafa does not own.

All rights not expressly granted by this license are reserved by PT NAKAFA TEKNO
KREATIF.

## 3. Allowed Personal Use

You may do the following without written permission:

- Read and view Nakafa content for personal, non-commercial learning.
- Download or print content only when an official Nakafa feature allows it.
- Keep one private copy for your own personal, non-commercial study.
- Quote short excerpts with clear attribution when allowed by law.
- Submit corrections or improvements under `.github/CONTRIBUTING.md`.

## 4. Permission Required

You must get prior written permission from PT NAKAFA TEKNO KREATIF before doing
anything not listed in Section 3.

The following uses always require prior written permission:

- Copying, publishing, mirroring, redistributing, uploading, reselling, or
  repackaging Nakafa content.
- Using Nakafa content in a paid product, paid course, tutoring service,
  school program, LMS, question bank, worksheet pack, test prep product,
  content site, newsletter, book, video, app, API, dataset, or SaaS product.
- Using Nakafa content to earn ads, sponsorships, affiliate revenue,
  subscription revenue, consulting revenue, training revenue, or other direct
  or indirect business benefit.
- Presenting Nakafa content as your own, rebranding it, translating it for
  redistribution, or removing Nakafa attribution.
- Bulk downloading, crawling, scraping, indexing, extracting, embedding, or
  building a searchable copy of Nakafa content.
- Training, fine-tuning, benchmarking, evaluating, seeding, or improving AI
  models, embeddings, retrieval systems, datasets, search products, or competing
  educational products with Nakafa content.

## 5. Attribution

If a use is allowed by this license or by law, attribution must be clear and
truthful:

`Source: Nakafa, https://nakafa.com`

Attribution does not grant permission for uses that this license prohibits.

## 6. Termination

Your rights under this license end immediately if you violate any term.

After termination, you must stop using the content and delete all copies under
your control, except copies you are required to keep for legal compliance.

PT NAKAFA TEKNO KREATIF may reinstate your rights only in writing.

## 7. Mandatory Legal Rights

This license does not limit rights that applicable law says cannot be waived,
limited, or conditioned by contract.

## 8. No Warranty

Nakafa content is provided "as is" and "as available", without warranties or
conditions of any kind, whether express, implied, statutory, or otherwise.

PT NAKAFA TEKNO KREATIF does not warrant that Nakafa content is accurate,
complete, uninterrupted, error-free, fit for a particular purpose, or
non-infringing.

## 9. Limitation of Liability

To the fullest extent permitted by law, PT NAKAFA TEKNO KREATIF will not be
liable for any indirect, incidental, special, consequential, exemplary, punitive,
or similar damages, or for any loss of profits, revenue, data, goodwill, or
business opportunity, arising from or related to Nakafa content or this license.

## 10. Governing Law and Venue

This license is governed by the laws of the Republic of Indonesia, without
regard to conflict of law principles.

Except where prohibited by applicable law, any dispute arising out of or related
to this license or Nakafa content must be brought exclusively in the courts of
the Republic of Indonesia.

## 11. No Waiver

If PT NAKAFA TEKNO KREATIF does not enforce a term immediately, that does not
waive the right to enforce that term later.

Any waiver must be in writing and signed by PT NAKAFA TEKNO KREATIF.

## 12. Severability

If any part of this license is found unenforceable, the rest remains in effect.
The unenforceable part will be interpreted as narrowly as possible so it becomes
enforceable while preserving the original intent.

## 13. Written Permission

Commercial, institutional, hosted, redistribution, rebrand, AI-training, dataset,
bulk extraction, or other non-personal use requires a separate written
agreement.

Contact: nakafaai@gmail.com
