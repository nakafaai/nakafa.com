...
Move verifyApiKey mutation from component to auth folder.

## Implementation

### 1. Create Auth Mutations File

Create `packages/backend/convex/auth/mutations.ts`:
...
export const verifyApiKey = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (currentRemaining !== null) {
      await ctx.db.patch("apikey", apiKey._id, { remaining: currentRemaining - 1 });
    }
...
// After:
import { verifyApiKey } from "@repo/backend/convex/auth/mutations";
```

## Verification

- [ ] Mutation moved to auth/mutations.ts
- [ ] Uses validatePermissions from auth/utils.ts
- [ ] Imports updated
- [ ] TypeScript compiles
