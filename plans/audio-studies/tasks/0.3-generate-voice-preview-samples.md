...
```json
{
  ...
  "steps": [
    "Create script generation action for samples",
...
- [ ] Script exists to generate voice samples
 ...
 * Run manually: npx convex run audioStudies/generateSamples.ts
 */

import { action } from "@repo/backend/convex/functions";
...
export const generateVoiceSamples = action({
  args: {},
...
**Output**: Action that generates and stores voice samples.
...
5 samples × ~100KB each = negligible storage cost. Much cheaper than generating on-demand.
