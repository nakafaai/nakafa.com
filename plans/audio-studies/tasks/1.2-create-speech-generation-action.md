...
This action takes the script with audio tags and generates speech using ElevenLabs. It stores the resulting audio in Convex storage and updates the contentAudios record.
...
3. Audio tags may affect actual duration slightly
...
- Good handling of audio tags

Alternative: `eleven_flash_v2_5` for faster generation (slightly lower quality).

### Error Handling

Uses `ConvexError` with proper codes:
- NOT_FOUND: Audio study doesn't exist
- BAD_REQUEST: Audio study exists but has no script (shouldn't happen in workflow)
