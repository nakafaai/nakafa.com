# Task 3.2: Create Get Audio Query (Public API)
...
Clients need to:
1. Check generation status (polling during generation)
...
```json
{
  ...
  "steps": [
    "Create getAudioStatus query for polling",
 ...
 * Called by UI to poll for completion during generation
 */
...
- Task 5.1: Testing these queries

## ⚠️ Important Notes

### Polling Strategy

UI should poll `getAudioStatus` during generation:
```typescript
// React example
...
useEffect(() => {
  if (data?.status === "generating") {
    const interval = setInterval(() => {
      // Convex query automatically re-fetches
