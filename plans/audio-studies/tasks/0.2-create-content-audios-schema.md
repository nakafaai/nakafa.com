...
Define the database schema for contentAudios table with proper indexes and validation using shared validators.

## 📍 Context

Following Convex best practices, we reuse validators from `lib/contentValidators.ts` and use proper indexing for query performance.
...
```json
{
  ...
  "steps": [
    ...
    "Use shared validators from lib/contentValidators.ts",
...
- [ ] Uses shared validators (audioStatusValidator, voiceSettingsValidator)
...
const tables = {
  ...
  contentAudios: defineTable({
    ...
    // Workflow status - uses shared validator
...
**Output**: Schema file with contentAudios and userContentAudios tables using shared validators.
...
- `audioStatusValidator` - Prevents duplication of status union
- `voiceSettingsValidator` - Prevents duplication of voice settings object
- `localeValidator` - Reuses existing locale validator
