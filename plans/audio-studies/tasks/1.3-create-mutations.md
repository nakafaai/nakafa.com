...
```typescript
import { mutation, internalMutation } from "@repo/backend/convex/_generated/server";
...
export const request = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const sub = await ctx.db.query("subscriptions")
      .withIndex("customerId_status", q => q.eq("customerId", customer._id).eq("status", "active"))
      .first();
    if (!sub) {
      throw new ConvexError({
        code: "FORBIDDEN",
    ...
    // Voice
    const voiceKey = isValidVoiceKey(args.voiceKey || "") 
      ? (args.voiceKey as typeof DEFAULT_VOICE_KEY) 
      : DEFAULT_VOICE_KEY;
    const voice = getVoiceConfig(voiceKey);

    // Content
    const content = args.contentType === "article"
      ? await ctx.db.get(args.contentId)
      : await ctx.db.get(args.contentId);
    if (!content) {
      throw new ConvexError({
        code: "NOT_FOUND",
    ...
    // Upsert record first
    const id = await ctx.runMutation(internal.audioStudies.mutations.upsert, {
      contentId: args.contentId,
      contentType: args.contentType,
    ...
    // Start durable workflow for generation
    await workflow.start(ctx, internal.audioStudies.workflows.generateAudio, {
      contentAudioId: id,
    });
