...
Create a Convex workflow that orchestrates the full audio generation pipeline: script generation → speech generation → completion.
...
```json
{
  ...
  "steps": [
    ...
    "Define workflow steps: create record → generate script → generate speech → complete",
...
- [ ] Orchestrates 3 steps: script generation → speech generation → completion
...
import { localeValidator, voiceSettingsValidator } from "@repo/backend/convex/lib/contentValidators";
...
2. If yes → return existing (cached)
3. If generating → return ID (user polls for completion)
4. If stale/failed → start new generation
