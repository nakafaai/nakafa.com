...
Verify the entire pipeline works: request → check cache → start workflow → generate script → generate speech → store → playback.
...
- [ ] Polling reflects status progression (pending → scripting → speech → completed)
...
### Subtask 5.1.1: Create Test Script
...
### Subtask 5.1.2: Manual Testing Checklist
...
After this task:
- Next: Task 5.2 (Test cache invalidation)
- Dependencies: Tasks 1-4 complete

## 🔗 Related Tasks

- Task 5.2: Testing invalidation specifically
