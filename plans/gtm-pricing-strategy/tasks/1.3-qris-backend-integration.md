...
**File**: `packages/backend/convex/schema.ts` (add)
...
**Output**: Database schema extended
...
**File**: `packages/backend/convex/crons.ts` (add)
...
**File**: `packages/backend/convex/qris/mutations.ts`
...
export const activateSubscription = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(internal.subscriptions.createFromQris, {
      userId: payment.userId,
      amount: payment.amount,
      currency: "IDR",
      periodStart: now.toISOString(),
      periodEnd: periodEnd.toISOString(),
      productId: "qris_pro_monthly",
      checkoutId: args.invoiceId,
      metadata: {
        paymentMethod: args.paymentMethod,
        customerName: args.customerName,
        source: "qris",
...
- ❌ Duplicates requests from multiple tabs
