...
1. Choose the project name and current official template. For a new app, run `npm create convex@latest <project-name>` and complete the scaffolder prompts. For an existing app, install `convex` and keep the current frontend structure.
2. Install dependencies if the scaffolder did not do so, then run `CONVEX_AGENT_MODE=anonymous npx convex dev --once` from the project. This creates an isolated local backend, pushes the schema and functions, generates types, and exits with validation results.
3. Start the project-owned dev command in the background, wait for its URL, and open it for the user.
4. Present a short plan and CONFIRM before building features beyond the template.

## Rules

- Never re-run the scaffolder after it has created the project successfully.
